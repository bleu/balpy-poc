...
def filter_swap_fee_changes_on_pool_creation(entries):
    """Filter out SwapFeePercentageChanged events when the it is generated by the pool creation transaction."""
    pool_registered_txs_hashs = [
        entry["transactionHash"] for entry in entries if parse_event_name(entry) == Event.PoolRegistered
    ]
    
    return [
        entry for entry in entries if entry["transactionHash"] not in pool_registered_txs_hashs or parse_event_name(entry) != Event.SwapFeePercentageChanged
    ]
        

def filter_entries(entries):
    filters = [
        filter_multiple_swap_fee_changes,
        filter_swap_fee_changes_on_pool_creation
    ]
...
async def test_block_range(chain: Chain, from_block: int, to_block: int):
    ...
    filtered_events = filter_entries(events)
